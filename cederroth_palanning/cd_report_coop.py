...
AVAILABLE_TARGETS = [('01',"Listingi"), ('02',"Hostessy"), ('03',"Programy lojalno�ciowe"), ('04',"Merchandising"), ('05',"Inne")]

class cd_report_coop(osv.Model):
    _name = "cd.report.coop"
    _auto = False
    ...
    def init(self, cr):
        tools.drop_view_if_exists(cr, 'cd_report_coop')
        cr.execute("""
            CREATE OR REPLACE VIEW cd_report_coop AS (
...
select
    ...
    'Akcje pozabudżetowe - COOP' as name,
    co.amount as cost,
    co.count,
...
where ccd.cost_type = '02' and cp.sequence not in (40,90)
UNION
...
WHEN cl.price_sale is not null then
    CASE
        WHEN cdp.discount_back_coop IS NOT NULL THEN (cl.price_sale - (cl.price_sale/100*cdp.discount_front))/100*cdp.discount_back_coop
        WHEN rp.discount_back_coop IS NOT NULL THEN (cl.price_sale - (cl.price_sale/100*rp.discount_front))/100*rp.discount_back_coop
        ELSE 0
    END
ELSE 0
END) as cost,
    ...
    CASE
        WHEN cdp.discount_back_coop IS NOT NULL THEN (cl.price_sale - (cl.price_sale/100*cdp.discount_back_coop))/100*cdp.discount_back_coop*cpp.plan_count
        WHEN rp.discount_back_coop IS NOT NULL THEN (cl.price_sale - (cl.price_sale/100*rp.discount_back_coop))/100*rp.discount_back_coop*cpp.plan_count
        ELSE 0
    END
        ELSE 0
        END) as value
